...
class AesopESRGANArtifactsDisMoEV2Model(SRGANModel):

    def __init__(self, opt):
        super(AesopESRGANArtifactsDisMoEV2Model, self).__init__(opt)
        if self.is_train:
    ...
    def optimize_parameters(self, current_iter):
        ...
        with amp.autocast('cuda', enabled=self.use_amp, dtype=self.amp_dtype):
            ...
            l_d_ortho = self.orthogonal_loss(ortho_w) * 10 # 10.0
            l_d_real += l_d_ortho
            loss_dict['l_d_ortho'] = l_d_ortho

            # Eq. 7: LDA distribution loss (λ_d = 10)
            l_d_dist = self.distribution_loss(features) * 10 #10.0
            l_d_real += l_d_dist
            loss_dict['l_d_dist'] = l_d_dist

            # Eq. 4: balancing loss (λ_b = 0.05) — uses soft R' from HR call
            l_d_balance = self.balancing_loss(self.net_d._R_prime, self.net_d.num_expert, routing_hard=routing) * 0.005 # 0.05
    ...
    def balancing_loss(self, R_prime, num_expert, routing_hard=None):
        """Switch-Transformer style load balancing.

        Allows per-pixel specialization (peaked softmax) while preventing
        aggregate expert collapse. Only penalizes when one expert dominates
        across the whole batch, not when individual pixels are confidently routed.

        Args:
            R_prime: Soft routing probabilities (B, N, H, W).
            num_expert: N.
            routing_hard: Hard routing output (B, N, H, W).
        """
        N = num_expert
        if routing_hard is not None:
            density = routing_hard.float().mean(dim=(0, 2, 3))  # (N,)
        else:
            hard = F.one_hot(R_prime.argmax(dim=1), N).permute(0, 3, 1, 2).float()
            density = hard.mean(dim=(0, 2, 3))
        density_proxy = R_prime.mean(dim=(0, 2, 3))  # (N,)
        return N * (density * density_proxy).sum()
