...
class EvalImagePairedDataset(data.Dataset):
    
    def __init__(self, opt):
        super(EvalImagePairedDataset, self).__init__()
        self.opt = opt
        ...
        with h5py.File(self.h5_file, 'r') as f:
            self._group = f[self.dataset] if self.dataset is not None else f
            self.paths = sorted(list(self._group.keys()), key=lambda k: int(k) if k.isdigit() else k)
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.h5_file, 'r') as f:
            self._group = f[self.dataset] if self.dataset is not None else f
            img_bytes = np.array(self._group[self.paths[index]])
        ...
        #return {'lq': img_lq, 'gt': img_gt, 'lq_path': (self.dataset + '-') if self.dataset is not None else '' + gt_path, 'gt_path': gt_path}
        return {'lq': img_lq, 'gt': img_gt, 'lq_path': ((self.dataset + '_') if self.dataset is not None else '') + f'{index}_' + gt_path, 'gt_path': gt_path}
